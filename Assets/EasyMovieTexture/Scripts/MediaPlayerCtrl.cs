...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    //sturgeon added
    public delegate void ActivityIndicatorShow(bool show);
    public static event ActivityIndicatorShow OnActivityIndicatorShow;
    //sturgeon added
	...
	void Update()
	{
		...
		if (m_bFirst == false)
		{
            ...
            Debug.Log("MediaPlayCtrl First: " + strName);
        ...
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                if (hit.transform.gameObject.name == this.name)
                {
                    if (!isChromakey)
                    {
                        Debug.Log("Click videoPanel, panel's name = " + this.name);
                        GameObject videoControlObject = this.transform.FindChild("VideoControl").gameObject;
                        videoControlObject.SetActive(!videoControlObject.activeInHierarchy);
                    }
                    else
                    {
                        if (GetCurrentState() == MEDIAPLAYER_STATE.PAUSED || GetCurrentState() == MEDIAPLAYER_STATE.STOPPED)
                        {
                            Play(); 
                        }
                        else if ( GetCurrentState() == MEDIAPLAYER_STATE.PLAYING)
                        {
                            Pause();
                        }
                    }
	...
	public void Load(string strFileName)
	{
		...
		//m_bFirst = false; sturgeon modified
		m_bCheckFBO = false;
    ...
    public float fCurrentSeekTime = 0.0f;
    double pts;
	...
	audioSource.time = fLastFrameTime;
                ...
                Material material = new Material(Shader.Find("Unlit/Unlit_YUV"));
	...
	public IEnumerator DownloadStreamingVideoAndLoad(string strURL)
	{
		...
		*/
            ...
            Debug.Log("MediaPlayCtrl saved path: " + write_path);
